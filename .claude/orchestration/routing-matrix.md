...
For any web app (web-app, static-site, SaaS, ecommerce, marketplace, internal-tool, and most API/full-stack work), load `.claude/orchestration/web-routing-matrix.md` — the elite-web-team map with operating modes (audit-only, fix-and-verify, production-hardening, ui-ux-polish, security-review, performance-review, release-readiness, documentation), task→specialist routing, and verification loops. Entry commands: `.claude/commands/web-audit.md` (scope+mode audit) and `.claude/commands/web-readiness.md` (ship gate against `.claude/checklists/web-production.md`). First step on any unfamiliar web repo: `.claude/agents/core/codebase-mapper.md`. New web specialists: `.claude/agents/engineering/playwright-e2e-engineer.md`, `.claude/agents/engineering/refactoring-specialist.md`, `.claude/agents/engineering/bug-fix-specialist.md`, `.claude/agents/quality/auth-permission-reviewer.md`. For UI/visual polish, prefer the installed `frontend-design` skill (anthropics/skills).

## Mobile projects → use the mobile routing layer
For any mobile app (React Native, Expo, Flutter, native iOS/Swift, native Android/Kotlin, KMP, Ionic/Capacitor) load `.claude/orchestration/mobile-routing-matrix.md` — platform detection, the elite-mobile-team map, operating modes (incl. **app-store-readiness**), and verification loops. Entry commands: `.claude/commands/mobile-audit.md` (scope+mode) and `.claude/commands/store-readiness.md` (App Store + Google Play gate against `.claude/checklists/app-store-readiness.md` + `.claude/checklists/mobile-production.md`). New mobile specialists: `.claude/agents/quality/mobile-security-auditor.md` (MASVS), `.claude/agents/engineering/mobile-release-engineer.md` (Fastlane/EAS/signing/store submit), `.claude/agents/engineering/mobile-e2e-engineer.md` (Maestro/Detox/XCUITest/Espresso). Per-framework specialists already exist under `.claude/agents/stack/mobile/*`. **PWA & Telegram Mini Apps are web tech** → use the web layer plus `.claude/presets/telegram-mini-app.md` (server-side `initData` validation is P0). **Mixed projects** (web dashboard + mobile app + backend): keep the web and mobile teams separate per surface; union only the shared backend/data work.

## Desktop projects → use the desktop routing layer
For any desktop app (Electron, Tauri, Flutter Desktop, Qt/GTK, .NET desktop, native macOS/Windows/Linux) load `.claude/orchestration/desktop-routing-matrix.md` — framework detection, the elite-desktop-team map, operating modes (incl. **desktop-packaging-readiness**, **auto-update-readiness**, store-readiness), per-OS caveats, and verification loops. Entry commands: `.claude/commands/desktop-audit.md` (scope+mode) and `.claude/commands/desktop-readiness.md` (ship gate against `.claude/checklists/desktop-production.md` + `.claude/checklists/desktop-distribution.md`). New desktop specialists: `.claude/agents/engineering/electron-engineer.md`, `.claude/agents/engineering/tauri-engineer.md`, `.claude/agents/engineering/desktop-release-engineer.md` (signing/notarization/installers/auto-update), `.claude/agents/quality/desktop-security-auditor.md` (Electron/Tauri shell + update integrity). Native macOS/Windows/Linux, Qt/GTK, Flutter Desktop, and .NET desktop route to `.claude/agents/engineering/desktop-engineer.md` + the matching stack engineer (kept as routing knowledge, not always loaded). **Mixed** (web + mobile + desktop + backend): keep each surface's team separate; union only shared backend/data work.

## Monorepo / multi-service projects → use the monorepo orchestration layer
Before classifying a single project type, check for monorepo/multi-service topology (workspace tooling — `pnpm-workspace.yaml`, `turbo.json`, `nx.json`, `lerna.json`, `rush.json`, Cargo `[workspace]`, `go.work`, Bazel — or multiple independent manifests under `apps/`, `packages/`, `services/`). If detected, load `.claude/orchestration/monorepo-routing-matrix.md` — it classifies **per package** (each package routes through this same table independently), separates root-scoped concerns (CI, shared lockfile/SCA, release) from package-scoped ones, uses the dependency graph to size the blast radius of a shared-package change, and aggregates gates so a change to one package doesn't force a full-repo re-audit. `/route` runs this detection as its Step 0.
